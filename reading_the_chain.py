...
def connect_to_eth():
	url = "https://mainnet.infura.io/v3/8e1c7f119b75474796d0170dd035522a"  # FILL THIS IN
...
def is_ordered_block(w3, block_num):
	...
	for tx in block.get('transactions', []):
		...
		if base_fee is not None:
			# EIP-1559 transaction, which can be type 0 or type 2, check fields and calculate priority fee
			if 'maxPriorityFeePerGas' in tx and 'maxFeePerGas' in tx:
				# Type 2 transaction, calculate priority fee accordingly
				priority_fee = min(tx['maxPriorityFeePerGas'], tx['maxFeePerGas'] - base_fee)
			else:
				# Type 0 transaction, calculate priority fee accordingly
				priority_fee = tx['gasPrice'] - base_fee
		else:
			# Pre EIP-1559 transaction, which means we can just use gasPrice directly
			priority_fee = tx['gasPrice']

		# Append the calculated priority fee to the transactions list, for ordering check
		transactions.append(priority_fee)

	# Check if transactions are sorted in decreasing order of priority fee
	if transactions == sorted(transactions, reverse=True):
		# change the ordered variable to True, and return it
		ordered = True
